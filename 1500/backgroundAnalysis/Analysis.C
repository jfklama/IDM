...
void Analysis(const char *channelFile, const char *fileCreate = "RECREATE", Double_t sigma = 1., Bool_t overlay = 1) {
  ...
  if(overlay) {
    if( channel.find("_hphm_") != string::npos ) {
			//strcpy(inputFile, ("/media/jfklama/Filip_USB3/CLIC/1.5TeV/rootSamples/signal/" + channel + "_detResp.root").c_str());
			strcpy(inputFile, ("/media/jfklama/Filip_USB3/CLIC/1.5TeV/rootSamples_withOverlay/" + channel + "_detResp.root").c_str()); // DefaultSelectedPFOs (CDR)
      category = "signal";
    }
    else {
			//strcpy(inputFile, ("/media/jfklama/Filip_USB3/CLIC/1.5TeV/rootSamples/background/" + channel + "_detResp.root").c_str());
			strcpy(inputFile, ("/media/jfklama/Filip_USB3/CLIC/1.5TeV/rootSamples_withOverlay/" + channel + "_detResp.root").c_str()); // DefaultSelectedPFOs (CDR)
      category = "background";
    }
  }
    ...
    if( channel.find("_hphm_") != string::npos ) {
      strcpy(inputFile, ("/media/jfklama/Nowy/moje/FUW/IDM/rootSamples_withOverlay/1500/" + channel + "_detResp.root").c_str());
      category = "signal";
    }
    else {
      strcpy(inputFile, ("/media/jfklama/Nowy/moje/FUW/IDM/rootSamples_withOverlay/1500/" + channel + "_detResp.root").c_str());
  ...
  // Tree header
   ...
   struct event_data {
    ...
    event->Branch( "data",&mydata, "jetsM_inv/F:jetsEnergy:jetsTheta:leptonEnergy:leptonTheta:leptonPt:jetDeltaTheta:jetDeltaPhi:EFsum:MET:missingMass:jet1Theta:jet2Theta:jet1Pt:jet2Pt:jet1E:jet2E:M_inv_Wlep" );
		...
		if(category == "signal"){
			//meanCS = 2.266286913; // all signal samples (CALCULATE!)
			meanCS = 3.468642467; // off-shell W mean CS
			//meanCS = 1.56389; // off-shell W mean CS for 3 full sim. scenarios (CALCULATE!)
			if(channel.find("_BP21_") != string::npos || channel.find("_BP23_") != string::npos ||
				 ...
				 meanCS = 7.153885; // on-shell W mean CS
				 //meanCS = 4.97247; // on-shell W mean CS for 2 full sim. scenarios (CALCULATE!)
		}

    myheader.weight = meanCS*luminosity/numberOfEntries;
   ...
   // Fill energy sum (all entries in the detector)
    ...
    for(Int_t iphoton=0; iphoton<nPhoton; iphoton++) {
      ...
      if( (photon2->P4()).E() > 10 )
		...
		Double_t EFlowForCut = EFlowSumSmeared.Pt();
...
//      continue;
    ...
    if( 2*atan(exp(-1*lepP4.Eta())) > (TMath::Pi() - 0.25) || 2*atan(exp(-1*lepP4.Eta())) < 0.25 )
      continue;
...
//      continue;
  ...
  TFile f("~/FUW/CLIC/IDM/1500/backgroundAnalysis/VarHistogramsAfterCuts_withOverlay.root",fileCreate);
  HList.Write();
  f.Close();
