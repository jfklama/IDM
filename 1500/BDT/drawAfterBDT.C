...
void drawAfterBDT(
    ...
    TFile *file1 = TFile::Open( ("/home/jfklama/FUW/CLIC/IDM/1500/BDT/outputRootFiles/" + dir + "/my_mva_selection_test_" + sample + ".root").c_str() );
    TTree *TestTree = (TTree*)file1->Get( ("dataset_" + mode + dir + "/TestTree").c_str() );
    TTree *TrainTree = (TTree*)file1->Get( ("dataset_" + mode + dir + "/TrainTree").c_str() );
    ...
    c1.Print( ("plotsAfterBDT/withOverlay/"+var+sample+"_after_withOverlay.pdf").c_str() );
}
...
void drawOvertrain(std::string channel = "HP17"){
    ...
    string fileDirStr = "/home/jfklama/FUW/CLIC/IDM/1500/BDT/outputRootFiles/my_mva_selection_test_" + channel + ".root";
...
void CalculateBackgroundEff(std::string shell_type = "BP21", std::string channel = "ll", Float_t cs = 1396.18, Float_t BDTCut = 0.0976) {
...
void PrintAllBgEff() {
  ...
  CalculateBackgroundEff("BP21", "ll", 1396.18, 0.0976);
  CalculateBackgroundEff("BP21", "qq", 2873.4, 0.0976);
  CalculateBackgroundEff("BP21", "qqll", 2739.2, 0.0976);
  CalculateBackgroundEff("BP21", "qqlv", 7054.67, 0.0976);
  CalculateBackgroundEff("BP21", "qqlvlv", 56.912, 0.0976);
  CalculateBackgroundEff("BP21", "qqlvvv", 40.1477, 0.0976);
  CalculateBackgroundEff("BP21", "qqqq", 1970.68, 0.0976);
  CalculateBackgroundEff("BP21", "qqqqlv", 177.428, 0.0976);
  CalculateBackgroundEff("BP21", "qqvv", 1520.1, 0.0976);

  cout << endl;
  cout << "       off-shell W:" << endl;

  CalculateBackgroundEff("HP17", "ll", 1396.18, 0.0952);
  CalculateBackgroundEff("HP17", "qq", 2873.4, 0.0952);
  CalculateBackgroundEff("HP17", "qqll", 2739.2, 0.0952);
  CalculateBackgroundEff("HP17", "qqlv", 7054.67, 0.0952);
  CalculateBackgroundEff("HP17", "qqlvlv", 56.912, 0.0952);
  CalculateBackgroundEff("HP17", "qqlvvv", 40.1477, 0.0952);
  CalculateBackgroundEff("HP17", "qqqq", 1970.68, 0.0952);
  CalculateBackgroundEff("HP17", "qqqqlv", 177.428, 0.0952);
  CalculateBackgroundEff("HP17", "qqvv", 1520.1, 0.0952);

}

void CalculateSignificance(std::string channel="BP21", Double_t signalEv = 9398., Double_t BDTCut = 0.17) {
  ...
  double expBackgEvAfter = backgroundEff*1078272;
...
void PrintAllSignificances() {
  ...
  CalculateSignificance("BP18", 13315, 0.17);
  CalculateSignificance("HP2", 614, 0.17);
  CalculateSignificance("HP3", 673, 0.17);
  CalculateSignificance("HP5", 176, 0.17);
  CalculateSignificance("HP6", 15, 0.17);
/*  CalculateSignificance("HP7", 13315, 0.17);
  CalculateSignificance("HP8", 13315, 0.17);
  CalculateSignificance("HP9", 13315, 0.17);
  CalculateSignificance("HP10", 13315, 0.17); */
  CalculateSignificance("HP11", 8309, 0.17);
  CalculateSignificance("HP12", 4317, 0.17);
  CalculateSignificance("HP13", 6391, 0.17);
  CalculateSignificance("HP14", 2533, 0.17);
  CalculateSignificance("HP15", 5457, 0.17);
  CalculateSignificance("HP16", 4809, 0.17);
  CalculateSignificance("HP17", 1578, 0.17);
  CalculateSignificance("HP18", 3698, 0.17);
  CalculateSignificance("HP19", 2767, 0.17);
  CalculateSignificance("HP20", 1484, 0.17);

  CalculateSignificance("BP21", 9398, 0.08);
  CalculateSignificance("BP23", 14581, 0.08);
  CalculateSignificance("HP1", 8869, 0.08);
  CalculateSignificance("HP4", 239, 0.08);
