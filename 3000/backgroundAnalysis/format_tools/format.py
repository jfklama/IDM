...
res.iloc[:,0] = [str.replace('_3000','') for str in res.iloc[:,0]]
...
for i in range(len(res.iloc[9:-1,0])):
    ratio_row = {'channel':'signal/backg. ('+res.iloc[i+9,0]+')',
                 'N_bef':round_sig(res.iloc[i+9,1] / res.iloc[32,1]), 'N_aft':round_sig(res.iloc[i+9,2] / res.iloc[32,2]),
...
table = table.drop(labels=[53,54],axis=0)
table = table.drop(labels=table.index[39:52],axis=0)
table = table.drop(labels=[33,36,37],axis=0)

table.iloc[:-6,-1] = [round_sig(i) for i in table.iloc[:-6,-1]]   # round efficiencies to 2 significant digits
table.iloc[0:33,1] = [round_sig(i,3) for i in table.iloc[0:33,1]] # round ev. numbers to 3 significant digits
table.iloc[0:33,2] = [round_sig(i,3) for i in table.iloc[0:33,2]] # round ev. numbers to 3 significant digits
# write eff. in %
table.iloc[:-5,-1] = ["{:.2%}".format(i) for i in table.iloc[:-5,-1]]
# add math characters
table.iloc[9:32,0] = '$H^{+}H^{-}$ (' + table.iloc[9:32,0] + ')'
