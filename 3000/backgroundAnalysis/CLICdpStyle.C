...
void CLICdpLabelV(Double_t x=0.20, Double_t y=0.86, std::string status="work in progress", Color_t color=kBlack);
...
void CLICdpLabelV(Double_t x, Double_t y, std::string status, Color_t color){

   TLatex l;
   l.SetNDC();
//   l.SetTextFont(42);
   l.SetTextFont(62);    // Helvetica bold
   ...
   if(status.c_str()[0] == '_')
       dy = 0.7*l2.GetTextSize();

   if(status.c_str()[0] == '^')
       dy = 0.7*l2.GetTextSize()+0.03;
...
void CLICdpLabel(Double_t x, Double_t y, std::string status, Color_t color){
  ...
  double dx = 0.37*l.GetTextSize()*label.length();
