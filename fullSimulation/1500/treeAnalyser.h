...
// found on file: rootSamples/signal/idm_HpHm_1500_BP21.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Int_t           njet;
   Double_t        e_min;
   Double_t        jet_Px[2];   //[njet]
   Double_t        jet_Py[2];   //[njet]
   Double_t        jet_Pz[2];   //[njet]
   Double_t        jet_E[2];   //[njet]
   ...
   virtual void                     Loop(double crossSection = 8.093, TString R = 1.0, Bool_t myjets = false);
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("rootSamples/signal/idm_HpHm_1500_BP21.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("rootSamples/signal/idm_HpHm_1500_BP21.root");
      }
...
void treeAnalyser::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("npfo", &npfo, &b_npfo);
   fChain->SetBranchAddress("njet", &njet, &b_njet);
   fChain->SetBranchAddress("e_min", &e_min, &b_e_min);
