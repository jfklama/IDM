...
#res.loc['Total bckg.',1:3] = res.iloc[5:,1:3].sum(axis=0)
tot_bg_row = {'channel':'tot. backg.',
              'N_bef':res.iloc[5:,1].sum(axis=0), 'N_aft':res.iloc[5:,2].sum(axis=0),
              'eff':round_sig(res.iloc[5:,2].sum(axis=0)/res.iloc[5:,1].sum(axis=0)), 'entries_aft':res.iloc[5:,-1].sum(axis=0) }
res = res.append(tot_bg_row,ignore_index=True)

# add a row with the sig/bg ratios
for i in range(len(sig_ch)):
    ratio_row = {'channel':'signal/backg. ('+sig_ch[i]+')',
                 'N_bef':round_sig(res.iloc[i,1] / res.iloc[14,1]), 'N_aft':round_sig(res.iloc[i,2] / res.iloc[14,2]),
...
table.iloc[0:15,1] = [round_sig(i,3) for i in table.iloc[0:15,1]] # round ev. numbers to 3 significant digits
table.iloc[0:15,2] = [round_sig(i,3) for i in table.iloc[0:15,2]] # round ev. numbers to 3 significant digits
# write eff. in %
table.iloc[:-5,-1] = ["{:.2%}".format(i) for i in table.iloc[:-5,-1]]
# add math characters
table.iloc[:5,0] = '$H^{+}H^{-}$ (' + table.iloc[:6,0] + ')'
table.iloc[5:14,0] = '$' + table.iloc[5:14,0] + '$'
table.iloc[5:14,0] = [str.replace('l',r'\ell') for str in table.iloc[5:14,0]]
table.iloc[5:14,0] = [str.replace('v',r'\nu') for str in table.iloc[5:14,0]]
