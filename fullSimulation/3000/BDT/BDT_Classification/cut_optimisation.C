...
void cut_optimisation(string chname="BP18",
		  ...
		  //string weight = "w*(Chi2sig>0.0)",
	          int iwid = 2, int icol = 2, int ipat=1, string label = "", // params for drawing efficiencies
	          ...
	          double xfmin=0.0, double xfmax=0.9,
	          double yfmin=0.0000005, double yfmax=0.2  //0.000999
	       )
{
...
*/
  ...
  if(!ch2  || label.length()==0)
    {
      if(!ch2)
         ch2 = new TCanvas("ch2","BDT histograms",620,20,600,450);
      else
	{
	 ch2->cd();
	 ch2->Clear();
     ...
     double xtab[2] = { xfmin, xfmax};
     double ytab[2] = { yfmin, yfmax};

     TGraph* myframe = new TGraph(2,xtab,ytab);
     myframe->SetNameTitle("myframe","");
     myframe->SetMarkerColor(8);

     myframe->GetXaxis()->SetTitle("Signal efficiency");
     myframe->GetYaxis()->SetTitle("Background efficiency");
  ...
  ch2->cd();
	...
	TGraph *kineff = new TGraph(nbin-ifirst,sigeffi+ifirst,bgeffi+ifirst);
	kineff->SetNameTitle("kineff","");

	kineff->SetLineWidth(iwid);
  kineff->SetLineColor(icol);
  kineff->SetLineStyle(ipat);

  kineff->Draw("L");

	TLegend *gleg = (TLegend *) gROOT->FindObject("gleg");

  if(!gleg)
    {
    gleg = new TLegend(0.2, 0.52, 0.43, 0.93);
    gleg->SetName("gleg");
    gleg->SetLineColor(0);
    gleg->SetFillStyle(0);
    }
  ...
  //if(label.length() > 0)
    //{
      gleg->AddEntry(kineff,label.c_str(),"L");
      //gleg->Draw();
    //}
