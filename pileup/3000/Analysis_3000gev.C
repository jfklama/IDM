...
#include "/home/jfklama/FUW/CLIC/IDM/pileup/3000/jet_clustering.C"
#endif
...
void Analysis(const char *channelFile, const char *fileCreate = "RECREATE", Double_t sigma = 1., Bool_t bigStatistics = 0) {
  ...
  char inputFile[channel.size() + 1];
	//strcpy(inputFile, "/home/jfklama/FUW/CLIC/IDM/pileup/3000/qqlv_noOverlay_newChargedEff.root");
	strcpy(inputFile, "/home/jfklama/FUW/CLIC/IDM/pileup/3000/HP17_overlay_tightSelected_batch20FullSelection10ns_64ev_Poisson_1_newChargedEff.root");
  category = "signal";
  ...
  std::string metNameStr = "metR" + Rvalue + "_" + channel;
	...
	std::string nIsoPhotonsNameStr = "nIsoPhotonsR" + Rvalue + "_" + channel;
	std::string isoPhotonEnergyNameStr = "isoPhotonEnergyR" + Rvalue + "_" + channel;
  ...
  char metName[metNameStr.size() + 1];
	...
	char nIsoPhotonsName[nIsoPhotonsNameStr.size() + 1];
	char isoPhotonEnergyName[isoPhotonEnergyNameStr.size() + 1];
  ...
  strcpy(metName, (metNameStr).c_str());
	...
	strcpy(nIsoPhotonsName, (nIsoPhotonsNameStr).c_str());
	strcpy(isoPhotonEnergyName, (isoPhotonEnergyNameStr).c_str());
  ...
  TH1 *h_met = new TH1F(metName, "", 140, 0.0, 1400.0);
	...
	TH1 *h_nIsoPhotons = new TH1F(nIsoPhotonsName, "", 50, 0.0, 50.0);
	TH1 *h_isoPhotonEnergy = new TH1F(isoPhotonEnergyName, "", 50, 0.0, 100.0);
  ...
  ///////////////////////
	...
	TFile *outputFile = new TFile(("/home/jfklama/FUW/CLIC/IDM/pileup/3000/" + channel + "DataPreselected.root").c_str(),"RECREATE");
...
*/
   ...
   // Fill energy sum (all entries in the detector) and PFO theta histograms
    ...
    for(Int_t iphoton=0; iphoton<nPhoton; iphoton++) {
      ...
      if( (photon2->P4()).E() > 10 )
        z++;
			h_isoPhotonEnergy->Fill( (photon2->P4()).E() );
      }

		h_nIsoPhotons->Fill(nPhoton);
...
*/
  ...
  h_met->SetMinimum(0.001);
	...
	h_nIsoPhotons->SetMinimum(0.001);
	h_isoPhotonEnergy->SetMinimum(0.001);
  ...
  HList.Add(h_met);
	...
	HList.Add(h_nPFOs);
	HList.Add(h_hadPIDs);
	HList.Add(h_nIsoPhotons);
	HList.Add(h_isoPhotonEnergy);
...
*/
	...
	//TFile f("~/FUW/CLIC/IDM/3000/backgroundAnalysis/VarHistogramsBeforeCuts_withSmear1.root",fileCreate);
	TFile f("/home/jfklama/FUW/CLIC/IDM/pileup/3000/VarHistogramsBeforeCuts_overlay.root",fileCreate);
  HList.Write();
  f.Close();
...
*/
  ...
  fstream log;
    ...
    log.open("/home/jfklama/FUW/CLIC/IDM/pileup/3000/table.txt", fstream::out | fstream::app);
    if (log.is_open()) {
...
/*  if(category=="signal")
    {//signature
    fstream log2;
      log2.open("/home/jfklama/FUW/CLIC/IDM/3000/backgroundAnalysis/signature_eff.txt", fstream::out | fstream::app);
      if (log2.is_open()) {
        log2 << channel << ":" << endl;
